...
## B3. Structured QuaRot-FFN 结构适配

如果 B2 成功，并且希望 PPL 结论更接近 QuaRot，应进入 B3。

按 QuaRot 图 3 做 FFN 结构适配：
...
## B4. Model-level FFN-only Fake Quant
...
Value 侧第一阶段可以先采用 attention-local 评估方式：对 $V$ 做旋转量化，再反旋回原空间计算 $O=P\hat{V}$，以便直接和 FP16 attention output 对比。
...
Value cache 可以先用 attention-local 路径：

$$
V\rightarrow V_H\rightarrow Q_{\text{KV}}(V_H)\rightarrow \hat{V}
$$

其中 $\hat{V}$ 表示 dequant 后再反旋回原 value 空间的近似值。

再计算：

$$
\hat{O}_{\text{attn}}=\text{softmax}(\hat{S})\hat{V}
$$

最后通过 fake-quantized o_proj：

$$
\hat{Y}=Q_A(\hat{O}_{\text{attn}}H)\,Q_W(H^TW_o)
$$

注意：

> C5 仍然是 fake quant 数值实验，不是 native low-bit attention kernel。
...
| Method | Attention Linear | KV cache | 说明 |
| --- | --- | --- | --- |
| FP16 | FP16 | FP16 | baseline |
| Attn Direct-Absmax W4A4 + Absmax K4V4 | direct W/A fake quant | absmax KV | 普通 attention baseline |
| Attn Rot-Absmax W4A4 + Hadamard-LM K4V4 | rotation + uniform W/A | rotated KV | 计算不变性 + 4-bit KV |
| Attn Rot-LM W4A4 + Hadamard-LM K4V4 | rotation + LM W/A | rotated KV | 同 bit 非均匀 codebook |
| Attn Rot-LM W3A4 + Hadamard-LM K3V4 | weight 降位宽 | key 降位宽 | 核心低 bit 组合 |
| Attn Rot-LM W4A3 + Hadamard-LM K4V3 | activation 降位宽 | value 降位宽 | activation/value 降位宽 |
| Attn Rot-LM W3A4 + Hadamard-LM K2+QJL,V4 | weight 降位宽 | QJL corrected key | C4 成功后再做 |
...
## Stage 4：Model-level FFN-only Fake Quant
...
如果这里成功，再考虑完整 QuaRot-style FFN 结构适配。
...
把 q/k/v/o 线性 fake quant 与 post-RoPE rotated KV cache quant 串起来。

先做 C5-layer：

| Method | Attention Linear | KV cache |
| --- | --- | --- |
| FP16 | FP16 | FP16 |
...
| --- | ---: | ---: |
| FP16 | | |
| Attention Direct-Absmax W4A4 + Absmax K4V4 | | |
| Attention Rot-Absmax W4A4 + Hadamard-LM K4V4 | | |
| Attention Rot-LM W4A4 + Hadamard-LM K4V4 | | |
| Attention Rot-LM W3A4 + Hadamard-LM K3V4 | | |
| Attention Rot-LM W4A3 + Hadamard-LM K4V3 | | |
| Attention Rot-LM W3A4 + Hadamard-LM K2+QJL,V4 | | |

核心判断：

> Attention structured quant 在 PPL 上是否接近 FFN-only quant；K2+QJL 是否能接近或优于 pure K3V4。
...
| Method | Attention Linear | KV cache | Layer output err | PPL | Accuracy | Interpretation |
| --- | --- | --- | ---: | ---: | ---: | --- |
| FP16 | FP16 | FP16 | | | | baseline |
...
| Attention Rot-LM W3A4 + Hadamard-LM K2+QJL,V4 | weight 降位宽 | QJL corrected key | | | | C4 成功后的组合 |

核心比较：

$$
\text{Attn Rot-LM W3A4 + K3V4}\stackrel{?}{\approx}\text{Attn Rot-Absmax W4A4 + K4V4}
$$
...
* `Attention Rot-LM W3A4 + Hadamard-LM K3V4` 是否接近 `Attention Rot-Absmax W4A4 + Hadamard-LM K4V4`；
* `K2+QJL,V4` 是否能接近或优于 pure `K3V4`。
