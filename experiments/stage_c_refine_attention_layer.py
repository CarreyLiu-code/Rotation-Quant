...
from rotationquant.stage_c_model import attention_layer_metrics_from_details, fake_quant_refine_attention_from_record


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage C refine local attention-layer experiments.")
    parser.add_argument("--model-dir", default=TINYLLAMA_BASE_DIR)
    parser.add_argument("--output-dir", default="outputs/stage_c_refine")
    parser.add_argument(
        "--methods",
        nargs="+",
        default=[
            ...
            "attn_rot_lm_w3a4_hlm_k3v4_oabsorb",
            "attn_rot_lm_w4a3_hlm_k4v3_oabsorb",
        ],
    )
...
def method_metadata(method_key: str) -> dict[str, object]:
    spec = STAGE_C_REFINE_ATTENTION_SPECS[method_key]
...
def main() -> None:
    ...
    args = parse_args()
    unknown = sorted(set(args.methods) - (set(STAGE_C_REFINE_ATTENTION_SPECS) | {"fp16"}))
    if unknown:
        raise ValueError(f"Unknown Stage C refine attention methods: {unknown}")

    output_dir, run_id, timestamp = create_run_output_dir(args.output_dir, "stage_c_refine_attention_layer")
    ...
    progress = tqdm(total=len(capture.records) * len(args.methods), desc="Stage C refine attention layer")
    for item in capture.records:
        ...
        for method_key in args.methods:
            if method_key == "fp16":
                ...
                output, details = fake_quant_refine_attention_from_record(
                    item,
    ...
    write_run_metadata(
        build_run_metadata(
            experiment="stage_c_refine_attention_layer",
            args=args,
