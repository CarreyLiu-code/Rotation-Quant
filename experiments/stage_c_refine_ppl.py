...
from rotationquant.stage_c import STAGE_C_KV_SPECS, STAGE_C_REFINE_ATTENTION_SPECS
from rotationquant.stage_c_model import apply_stage_c_refine_attention_


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage C refine Attention-only model-level PPL evaluation.")
...
def write_summary(records: list[dict[str, object]], output_dir: Path) -> None:
    ...
    lines = ["# Stage C Refine PPL Summary", "", "| " + " | ".join(columns) + " |", "| " + " | ".join("---" for _ in columns) + " |"]
...
def main() -> None:
    ...
    if unknown:
        raise ValueError(f"Unknown Stage C refine PPL methods: {unknown}")

    output_dir, run_id, timestamp = create_run_output_dir(args.output_dir, "stage_c_refine_ppl")
    ...
    for method_name in args.methods:
        ...
        if method_name != "fp16":
            quant_metadata = apply_stage_c_refine_attention_(
                model,
                method_name,
